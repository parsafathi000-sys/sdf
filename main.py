...
CONFIG = {
    ...
    # استفاده از مسیر مطلق با دایرکتوری data
    "db_path": os.environ.get("DB_PATH", os.path.join(DATA_DIR, "panel.db")),
    ...
    async def init_db():
        ...
        try:
            ...
            db_conn = await aiosqlite.connect(
                CONFIG["db_path"],
                timeout=30  # افزایش تایم‌اوت برای جلوگیری از خطا
            )
            db_conn.row_factory = aiosqlite.Row
            
            # فعال کردن WAL برای عملکرد بهتر
            await db_conn.execute("PRAGMA journal_mode=WAL")
            await db_conn.execute("PRAGMA synchronous=NORMAL")
            await db_conn.execute("PRAGMA cache_size=-10000")  # 10MB cache
